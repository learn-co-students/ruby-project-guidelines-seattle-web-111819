...
def write_review(user, game, tracer=2)
    display_menu_header(["My Review of", "'#{game.name}'."], user)
    rating = 0
    message = []
    ...
    review_text = "<EMPTY>" if review_text.split(' ').count == 0
...
# (ROUTER 16):
# Update the User's Review rating
